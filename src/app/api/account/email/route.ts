...
import { changeEmailSchema } from "@/lib/account-security";
...
export async function PATCH(request: Request) {
  try {
    ...
    const parsed = changeEmailSchema.safeParse(await readJsonBody(request, 8 * 1024));
    ...
    if (error) return NextResponse.json({ error: "E-Mail-Adresse konnte nicht geändert werden" }, { status: 400 });
