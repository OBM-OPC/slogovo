...
- Local validation passed: npm 10 clean install, lint, type-check, database validation (10 migrations), content validation (12 modules, 60 lessons, 9 grammar topics; 0 errors/warnings), 67 Vitest files / 230 tests, production build (110 static/dynamic routes), and `git diff --check`. Next 16's local-origin restriction was covered with a narrow `127.0.0.1` development allowlist; the four auth E2E journeys and the full lesson retry/sync/restore journey passed. A transient speaking-page fade that reduced muted-text contrast during the automated axe scan was removed. The telemetry API-request test now supplies the same-origin header required by the production request-security policy.
...
- #112 verification: Android Chrome Pixel 7 and iPhone WebKit emulation both passed the complete 14-route rendered-screen overflow/44px-target audit; 18 focused layout/dialog/exercise tests and the dedicated focus-scroll test passed. Final CI run `29371746129` passed all 15 Playwright journeys across Chromium, Android Chrome, and iPhone WebKit plus the production build.
...
- #112 WebKit root fix: trace inspection proved the development CSP's production-only `upgrade-insecure-requests` directive made WebKit rewrite every HTTP Next.js CSS/font/script asset to HTTPS on loopback, preventing hydration. Development/test HTTP now omits only that directive; production retains it and the nonce/strict CSP is unchanged. The audit retries only explicit Next-dev Fast Refresh navigation interruptions. The complete iPhone WebKit audit passed locally and in CI; #112 is closed.
...
- Final regression at `b0f8f86`: 67 Vitest files / 229 tests passed; type-check, lint, database validation, content validation (12 modules, 60 lessons, 9 grammar topics, 0 errors/warnings), high-severity audit, `git diff --check`, 111-page production build, 14 local Chromium/Android browser tests, and the local iPhone WebKit audit passed. GitHub Actions CI `29371746129` and Security `29371746223` passed, including dependency/secret scan, database security, and ZAP baseline.
- All 15 sub-issues #101–#115 are implemented, verified, pushed to `main`, and closed. Only parent #100 remains open for final status publication and closure.
